...
echo "[reset] corrigindo vet bug em report.go (Entries analyzed: %s → %d)..."
python3 - << 'PYEOF'
content = open("internal/report/report.go").read()
old = 'fmt.Fprintf(tw, "  Entries analyzed:\\t%s\\n", stats.Total)'
new = 'fmt.Fprintf(tw, "  Entries analyzed:\\t%d\\n", stats.Total)'
...
echo ""
echo "✓ Pronto! Estado inicial restaurado com bugs corrigidos:"
echo ""
echo "  ① processor.go → Avg() sem indentação (go fmt vai corrigir)"
echo "  ② report.go    → Entries analyzed: %d para int stats.Total (vet bug corrigido)"
echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
echo "  29. go list ./... && go list -m -u all | grep '\\['"
...
echo "  34. go mod tidy && cat go.mod    # remove color (não usada no código)"
